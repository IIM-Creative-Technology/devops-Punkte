...
export class VideoService {
  ...
  async findAll(getVideosQuery: GetVideosQuery) {
    ...
    if (getVideosQuery.videoCategoryId !== undefined) {
      videos = videos.andWhere('videoCategory.id = :videoCategoryId', {
        videoCategoryId: getVideosQuery.videoCategoryId,
      });
    }
    if (getVideosQuery.regionId !== undefined) {
      videos = videos.andWhere('regions.id = :regionId', {
        regionId: getVideosQuery.regionId,
      });
    }
    ...
    if (getVideosQuery.filters !== undefined) {
      videos = videos.select([
        'video.id',
        ...getVideosQuery.filters.map(filter => 'video.' + filter),
      ]);
    }
